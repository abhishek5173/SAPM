...
export default function Portfolio() {
  ...
  useEffect(() => {
    axios
      .get("https://script.googleusercontent.com/macros/echo?user_content_key=XFbb6r627vwBTWNdOBTDSeL0KQYrq-qcZ0yIS5VE0LD8_JG-uOkhsq1LRatZHjruHVpn2SVehCYxvoEZrqEn2wGPG-hQxknYm5_BxDlH2jW0nuo2oDemN9CCS2h10ox_1xSncGQajx_ryfhECjZEnO7vxK_gh2q46FW8N6O32tMyaCbTi7OtvoWObHj5wLmMqVHJHpcJNUK0vEe8zNmHvhl9PhPIIbUVdk4NPkHSl66Y6h1Fe1rV9dz9Jw9Md8uu&lib=MfEcSXofpCfK_wuKbVXUkRi91h4gJfZU7")  
      .then((response) => {
        const transformedData: SectorStocks = {};
        response.data.data.forEach((item: { Sector: string; Stock: string; Price: number }) => {
          transformedData[item.Sector] = transformedData[item.Sector] || [];
          transformedData[item.Sector].push({ stock: item.Stock, price: item.Price });
        });
  ...
  const handleStockSelect = (event: React.ChangeEvent<HTMLSelectElement>, sector: string) => {
    const stockName = event.target.value;
    if (!stockName) return;

    const stockData = sectorsWithStocks[sector].find((s) => s.stock === stockName);
    if (!stockData) return;

    setSelectedStocks((prev) => {
      const existingStock = prev.find((s) => s.stock === stockName);
      if (existingStock) {
        return prev.map((s) => (s.stock === stockName ? { ...s, quantity: s.quantity + 1 } : s));
      }
  ...
  const updateQuantity = (index: number, quantity: number) => {
    setSelectedStocks((prev) =>
      prev.map((s, i) => (i === index ? { ...s, quantity: Math.max(1, quantity) } : s))
    );
  };

  const resetSelections = () => setSelectedStocks([]);
  const totalPrice = selectedStocks.reduce((acc, s) => acc + s.price * s.quantity, 0);
      ...
      <div className="w-full min-h-screen bg-white flex flex-col items-center py-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 p-4 w-full max-w-7xl">
          {Object.keys(sectorsWithStocks).map((sector, index) => (
            <div key={index} className="flex flex-col items-center bg-gray-100 p-4 rounded-xl shadow-lg w-full">
  ...
  <div className="fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 p-4">
    <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl">
      ...
      <div className="mt-4 flex flex-col sm:flex-row justify-between items-center">
        <span className="text-lg font-semibold mb-2 sm:mb-0">Total: ₹{totalPrice}</span>
